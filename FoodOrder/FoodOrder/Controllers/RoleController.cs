...
namespace FoodOrder.Controllers
{
    public class RoleController : Controller
    {
        ...
        public async Task<ActionResult> Create(CreateRoleModel model)
        {
            ...
            {
                IdentityResult result = await RoleManager.CreateAsync(new RoleViewModel
                {
                    Name = model.Name,
        ...
        {
            RoleViewModel role = await RoleManager.FindByIdAsync(id);
            if (role != null)
            {
        ...
        public async Task<ActionResult> Edit(EditRoleModel model)
        {
            if (ModelState.IsValid)
            {
                RoleViewModel role = await RoleManager.FindByIdAsync(model.Id);
                if (role != null)
                {
